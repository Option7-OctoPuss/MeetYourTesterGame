...
class TestProgressBar:
	...
	func test_progress_bar_effect():
		...
		assert_eq(progress_bar_node.find_child(progress_bar_node_name).value, target_answer.effects.value+10) #added 10 because the progress bar for testing starts from 10
